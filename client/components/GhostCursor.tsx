...
const GhostCursor = ({
  ...
  const fragmentShader = `
    ...
    vec4 blob(vec2 p, vec2 mousePos, float intensity, float activity) {
      ...
      vec3 colorVal = mix(c1, c2, sin(iTime * 0.5) * 0.5 + 0.5);

      return vec4(colorVal * alpha * intensity, alpha * intensity);
    }
      ...
      const scale =
        need <= pixelBudget
          ...
          : Math.max(
              ...
              Math.min(
                1,
                Math.sqrt(pixelBudget / Math.max(1, need))
              )
            );
  ...
  useEffect(() => {
    if (materialRef.current) {
      ...
      materialRef.current.uniforms.iBaseColor.value.set(
        c.r,
        c.g,
        c.b
      );
  ...
  useEffect(() => {
    if (materialRef.current) {
      materialRef.current.uniforms.iEdgeIntensity.value =
        edgeIntensity;
    }
  }, [edgeIntensity]);
